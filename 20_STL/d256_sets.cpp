...
// kopyalşama (duplication) olmaz
// modifiye edilemez
// reverse yok

// std::unordered_multiset<int> set2;

// kopyalşama (duplication) olur
// modifiye edilebilir
// reverse yok
...
void test3() {
    ...
    std::set<std::string> s {"D", "B", "C"};
    display(s);
    
    auto result = s.insert("A");
    display(s);
    ...
    std::cout << std::boolalpha;
    std::cout << "first: " <<  *(result.first)<< std::endl; // eklenen harf
    

    std::cout  << "second: " << result.second << std::endl; // eklenip eklenmediği
}
